...
- Add standalone Azure Machine Configuration instructions to the README and a
  complete mdBook deployment guide covering MOF resolution, packaging, local
  testing, Storage, Azure Policy, assignment, verification, and
  troubleshooting.
- Remove nonexistent Apply-button guidance, internal tracker details, obsolete
  private-repository wording, and unused AI-analysis terminology from public
  documentation.

## [0.3.97] - 2026-07-28

> `v0.3.97` is the prior Windows/Linux tagged source. Its matching GitHub
...
> `v0.3.96` is the prior Windows/Linux tagged source. Its matching GitHub
> release was an unpublished draft.
...
- **Fix standalone Windows Server 2025 baseline audits.** PR
  [#83](https://github.com/Azure/ConfigForge/pull/83) ports the reviewed
  Member Server, Domain Controller, and Workgroup Member repairs to the macOS
  Author line. The corrected resource counts are 320, 321, and 296.
- **Remove misleading Source links from the corrected WS2025 baselines.** The
  local manifests now differ materially from their upstream files; manifest
  locations and scenario identities remain unchanged.
- **Improve CIS mappings for corrected non-registry settings.** Benchmark
  Mapping now resolves Increase scheduling priority, password complexity, and
  guest account status without bundling licensed CIS data.
- **Keep User Rights Assignment and Account Policy settings distinct.**
  Analysis and Matrix Diff use `properties.name`, with a legacy
  `properties.policy` fallback, instead of collapsing settings into a
  type-only row.
...
- **Updated PostCSS to 8.5.23 and `tar` to 7.5.22.** These patches prevent previous source-map path traversal and uncontrolled recursion in crafted archive member selection (GHSA-r28c-9q8g-f849, GHSA-r292-9mhp-454m).

## [0.3.91] - 2026-07-24

### Added

- **Surfaced Test schema rules directly in Visual Expected value cells.** `const`, `enum`, range, type, and supported composition rules now render as compact stacked technical rows, with each allowed enum value on its own line.

### Fixed

- **Aligned Visual edits with Test schema constraints.** Newly edited governed values are rejected when they violate supported rules, while existing shipped mismatches remain viewable and unrelated edits stay unblocked.
- **Repaired typed Registry and CSP values before serialization.** Declared types now override malformed string or boolean values, MultiString edits emit strings, and valid numeric edits are written as numeric YAML.
- **Bounded complex schema behavior.** Recursive/shared YAML aliases remain compact, unsafe regex patterns are labeled reference-only, and invalid YAML errors take precedence over value constraints.

## [0.3.90] - 2026-07-23

### Fixed

- **Prevented Baseline Detail footer actions from colliding at scaled and localized widths.** The fully labeled action bar now reflows to two rows before the center strip can overlap Close baseline or Edit/Save, while sufficiently wide windows remain on one row.
- **Contained long multi-value rows inside their Visual table columns.** Read-only Expected and Applied array values now use the same anywhere-wrapping behavior as edit mode, preventing cipher suites, hardened-path directives, and similar values from painting across adjacent cells.
...
- **Baseline Detail now presents baselines as documents.** A white-surface document icon replaces the operating-system mark beside the page title, while the platform remains visible in its metadata badge.
- **Visual authoring actions are easier to reach.** Per-category actions are consistently named Add setting and appear above and below every settings table.
...
- **Made device compliance easier to find.** The baseline footer and My Baselines compliance statuses open a focused compliance drawer, while the permanent device-audit section remains available in the page.
...
- **Improved spreadsheet keyboard authoring.** Pressing Enter in the final editable cell commits the value and appends another row of the same setting type.
- **Normalized baseline footer typography.** Custom link actions now use the same text size as Fluent small buttons.

### Fixed
- The OSConfig install dialog no longer renders on top of the first-run Welcome dialog.
- OSConfig recheck skips spawn probes for missing non-alias install paths, avoiding repeated timeout-prone checks after installation.
