...
|---|---|---|
| `main` | `v0.3.97` | Current Full-edition Windows/Linux tagged source. Its matching GitHub release remains an unpublished draft with detailed CLI compliance reasons. |
| `mac-author-build` | `mac-v0.3.97-author.1` | Current author-only macOS tagged source with the same reason templates and authoring metadata. Its matching GitHub release remains an unpublished draft. |

On `mac-author-build`, the root package, desktop package, and matching lockfile
records use `0.3.97-author.1`. This release carries no dependency changes from
the prior macOS line. The package version on `main` is `0.3.97`; do not copy
...
- The current macOS Author tagged source `mac-v0.3.97-author.1` expects
...
- The prior annotated tag `mac-v0.3.94-author.1` resolves to
  ...
  [#30233283418](https://github.com/Azure/ConfigForge/actions/runs/30233283418),
  but that release remains a draft and unpublished.
- The current macOS Author tagged source is `mac-v0.3.97-author.1`. Its
