...
async function readComplianceSummary(
  ...
  const audit = await readAuditResult(namespace);
...
async function buildManifestList(
  ...
  return Promise.all(
    Array.from(allNames).map(async (name) => {
      ...
      if (reg) {
        const compliance = await readComplianceSummary(name);
        ...
        const base: Omit<ManifestListEntry, 'Resources'> = {
          ...
          LastModifiedAt: reg.registeredAt ?? null,
        };
...
export async function getManifest(
  ...
  const resources = (summary ?? []).map((r) => ({ name: r.name, type: r.type }));
  const compliance = await readComplianceSummary(namespace);
  const data = {
    Name: namespace,
...
export async function registerManifest(
  ...
  // Soft platform warnings. On macOS (author flavor) there is no deploy
  // capability, so platform-mismatch and unregistered-type warnings are
  // noise — every manifest would trigger them. Skip them entirely.
  ...
  // v0.3.0 (#20): namespace-collision warning.
  if (!req.force) {
  ...
  if (!isDarwin && (manifestPlatform === 'mixed' || hasMixedPlatformResources(resources))) {
    warnings.push(
  ...
  if (!isDarwin && (manifestPlatform === host || manifestPlatform === 'cross-platform')) {
...
export async function restoreManifest(
  req: RestoreManifestRequest,
): Promise<RestoreManifestResult> {
