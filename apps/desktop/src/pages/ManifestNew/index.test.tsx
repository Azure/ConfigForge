...
describe("ManifestNewPage post-registration refresh", () => {
  ...
  it("cancels delayed batch navigation when the page unmounts", async () => {
    ...
    try {
      unmount();
      expect(clearTimeoutSpy.mock.calls.some(([timer]) => timer === navigationTimer)).toBe(true);
    } finally {
  ...
  it("rejects an oversized single file before reading it", async () => {
    ...
    expect(
      await screen.findByText(
        "oversized.yaml: The baseline is larger than the 10 MB import limit.",
      ),
    ).toBeInTheDocument();
  ...
  it("bounds batch import concurrency", async () => {
    ...
    vi.mocked(window.cfs!.manifests.register).mockRejectedValue(new Error("Expected test failure"));
    ...
    await waitFor(() => expect(window.cfs!.importChannel.fromContent).toHaveBeenCalledTimes(4));
    expect(maxActiveImports).toBe(4);

    await act(async () => {
      releases.splice(0).forEach((release) => release());
    });
    await waitFor(() => expect(window.cfs!.importChannel.fromContent).toHaveBeenCalledTimes(6));
    await act(async () => {
      releases.splice(0).forEach((release) => release());
    });
  ...
  it("keeps normal XLSX imports working within the size limit", async () => {
    ...
    const arrayBufferSpy = vi.spyOn(workbook, "arrayBuffer").mockResolvedValue(bytes.buffer);
  ...
  it("ignores a pending URL fetch after the creation method changes", async () => {
    ...
    expect(screen.getByRole("radio", { name: /Create my own baseline/i })).toBeChecked();
    expect(screen.queryByRole("button", { name: "Visual" })).not.toBeInTheDocument();
  });
