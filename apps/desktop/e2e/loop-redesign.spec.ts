...
test.describe.serial('Loop redesign end-to-end flow', () => {
  ...
  test('navigation counts and My Baselines table reflect authoritative data', async () => {
    ...
    await expect(
      page.getByRole('button', { name: `Open baseline ${BASELINE_LINUX}` }),
    ).toBeVisible();
  ...
  test('search filters baseline identity without hidden resource collisions', async () => {
    ...
    await expect(page.getByRole('button', { name: `Open baseline ${BASELINE_BLANK}` })).toHaveCount(
      0,
    );
  ...
  test('Code and Visual viewers preserve read-only and edit flows', async () => {
    ...
    const narrowLayout = await registryScrollArea.evaluate((element) => {
      ...
      const overlaps = visibleRects.some(
        (rect, index) => index > 0 && visibleRects[index - 1].right > rect.left + 1,
      );
    ...
    await visual.getByRole('button', { name: 'Edit Applied value for AlphaSetting' }).click();
    ...
    await visual.getByRole('button', { name: 'Edit Applied value for AlphaSetting' }).click();
    await visual.getByRole('textbox', { name: 'Edit Applied value for AlphaSetting' }).fill('5');
  ...
  test('Benchmark Mapping transitions in place from unloaded to usable CIS data', async () => {
    ...
    const xccdfPrefix = 'CIS_Azure_Compute_Microsoft_Windows_Server_2022_Benchmark_v1.0.0';
    await Promise.all([
      writeFile(
      ...
      writeFile(path.join(dataDir, `${xccdfPrefix}-oval.xml`), '<oval_definitions />', 'utf8'),
      writeFile(path.join(dataDir, `${xccdfPrefix}-cpe-oval.xml`), '<oval_definitions />', 'utf8'),
    ]);
  ...
  test('persisted compliance survives page navigation and application restart', async () => {
    ...
    await page.getByRole('button', { name: `Open compliance for ${BASELINE_A}` }).click();
    ...
    await page.getByTestId('manifest-detail-footer').getByRole('button', { name: 'Save' }).click();
    await page
