...
unpublished draft release. Use its current GitHub checks and release metadata
as the authority rather than pinning a workflow run in this document.
...
The `mac-v0.3.94-author.1` release contract expects exactly these assets:
...
remains a draft and is unpublished. For `mac-v0.3.94-author.1`, use the
current GitHub checks and draft release as the authority for actual build and
asset status.
